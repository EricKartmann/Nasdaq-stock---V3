...
# Crear pestañas
tab1, tab2 = st.tabs(["Corto Plazo", "Largo Plazo"])
...
def calculate_ema(data, periods):
    """Calcula el EMA para una serie de datos"""
    return data.ewm(span=periods, adjust=False).mean()

def get_long_term_signals(hist_daily):
    """
    Implementa estrategia de trading basada en:
    - Cruce de EMAs (3 y 30 días)
    - Tendencia con EMA 50
    - RSI para zonas de sobrecompra/sobreventa
    - MACD para confirmar tendencia
    """
    # Calcular EMAs
    ema_3 = calculate_ema(hist_daily['Close'], 3)
    ema_30 = calculate_ema(hist_daily['Close'], 30)
    ema_50 = calculate_ema(hist_daily['Close'], 50)
    
    # Calcular RSI
    rsi = calculate_rsi(hist_daily, periods=14)
    
    # Calcular MACD - corregido para usar solo los valores necesarios
    macd, signal, hist, _ = calculate_macd(hist_daily, 
                                         short_window=12,
                                         long_window=26,
                                         signal_window=9)
    ...
    ema_cross = ema_3.iloc[-1] > ema_30.iloc[-1]
    prev_ema_cross = ema_3.iloc[-2] > ema_30.iloc[-2]
    price_above_50 = current_price > ema_50.iloc[-1]
    current_rsi = rsi.iloc[-1]
    macd_hist_positive = hist.iloc[-1] > 0
    
    # Determinar tendencia principal
    if price_above_50 and macd_hist_positive:
    ...
    signals.append(f"Tendencia {trend.lower()}")
    
    # Analizar señales de entrada/salida
    if trend in ["ALCISTA FUERTE", "ALCISTA"]:
        if ema_cross and not prev_ema_cross:  # Cruce alcista reciente
            if 30 <= current_rsi <= 70:  # RSI en zona neutral
                recommendation = "COMPRA FUERTE"
                signals.append("Cruce alcista EMA3/EMA30 | RSI en zona neutral")
            elif current_rsi < 30:
                recommendation = "COMPRA FUERTE"
                signals.append("Cruce alcista EMA3/EMA30 | RSI en sobreventa")
            else:
                recommendation = "MANTENER"
                signals.append("RSI en sobrecompra - esperar retroceso")
        elif ema_cross:  # Ya estamos en tendencia alcista
            if current_rsi < 40:
                recommendation = "COMPRA MODERADA"
                signals.append("Retroceso en tendencia alcista")
            else:
                recommendation = "MANTENER LARGO"
                signals.append("Continuación tendencia alcista")
        else:
            if current_rsi < 30:
                recommendation = "ACUMULAR"
                signals.append("Sobreventa en tendencia alcista")
            else:
                recommendation = "ESPERAR"
                signals.append("Esperando nuevo cruce alcista")
    else:  # Tendencia bajista o neutral
        if current_rsi < 30 and macd_hist_positive:
            recommendation = "ACUMULAR"
            signals.append("Sobreventa con MACD positivo")
        elif not ema_cross and current_rsi > 70:
            recommendation = "VENTA"
            signals.append("Sobrecompra en tendencia bajista")
        else:
            recommendation = "ESPERAR"
            signals.append("Esperando cambio de tendencia")
    
    # Añadir detalles técnicos
    signals.append(f"RSI: {current_rsi:.1f}")
    signals.append(f"Precio vs EMA50: {'Por encima' if price_above_50 else 'Por debajo'}")
    signals.append(f"MACD Histograma: {'Positivo' if macd_hist_positive else 'Negativo'}")
    
    return recommendation, " | ".join(signals)

def get_long_term_data(symbols):
    """Obtiene datos para análisis a largo plazo"""
    data = []
    for symbol in symbols:
        try:
            stock = yf.Ticker(symbol)
            ...
            recommendation, signals = get_long_term_signals(hist)
            
            # Determinar el estado con doble punto para señales fuertes
            if "COMPRA FUERTE" in recommendation:
                status = "🟢🟢"
            elif "COMPRA MODERADA" in recommendation or "MANTENER" in recommendation:
                status = "🟢"
            elif "ACUMULAR" in recommendation:
                status = "🟡"
            elif "VENTA FUERTE" in recommendation:
                status = "🔴🔴"
            else:
                status = "🔴"
            
            month_3_change = ((hist['Close'][-1] - hist['Close'][-63]) / hist['Close'][-63] * 100) if len(hist) >= 63 else 0
            month_6_change = ((hist['Close'][-1] - hist['Close'][-126]) / hist['Close'][-126] * 100) if len(hist) >= 126 else 0
            
            data.append({
                'Estado': status,
                'Símbolo': symbol,
                'Empresa': top_companies[symbol],
                'Precio Actual ($)': hist['Close'].iloc[-1],
                'Cambio 3 Meses (%)': round(month_3_change, 2),
                'Cambio 6 Meses (%)': round(month_6_change, 2),
                'Recomendación LP': recommendation,
                'Señales': signals
            })
        except Exception as e:
            st.error(f"Error procesando {symbol}: {str(e)}")
...
# Dentro de la primera pestaña (Corto Plazo)
with tab1:
...
# Dentro de la segunda pestaña (Largo Plazo)
with tab2:
    df_long = get_long_term_data(list(top_companies.keys()))
        ...
        st.dataframe(
            ...
            column_config={
                'Estado': st.column_config.Column(
                    "Estado",
                    help="🟢🟢 Compra Fuerte | 🟢 Compra/Mantener | 🟡 Acumular | 🔴 Venta | 🔴🔴 Venta Fuerte"
                ),
                'Cambio 3 Meses (%)': st.column_config.NumberColumn(
                    format="%.2f%%",
                    help="Cambio porcentual en los últimos 3 meses"
                ),
                'Cambio 6 Meses (%)': st.column_config.NumberColumn(
                    format="%.2f%%",
                    help="Cambio porcentual en los últimos 6 meses"
                )
            }
        )
